...
def save_model_card(repo_id: str, imgs=None, base_model=str, train_text_encoder=False, prompt=str,
                    repo_folder=None, pipe: DiffusionPipeline = None):
    img_str = ""
...
def log_validation(text_encoder, tokenizer, unet, vae, args, acc, weight_dtype, epoch, prompt_embeds,
    ...
    pipe = DiffusionPipeline.from_pretrained(# create pipe (NOTE: unet and vae are loaded again in float32)
    ...
    if args.precompute_text_embeds:
        pipe_args = {"prompt_embeds": prompt_embeds, "negative_prompt_embeds": negative_prompt_embeds}
    else:
        pipe_args = {"prompt": args.val_prompt}

    gen = None if args.seed is None else torch.Generator(device=acc.device).manual_seed(args.seed)
    imgs = []
    if args.val_imgs is None: # run inference
        ...
        for img in args.val_imgs:
            img = pipe(**pipe_args, image=Image.open(img), generator=gen).images[0]
            imgs.append(img)
...
def import_model_class_from_model_name_or_path(pretrained_model_name_or_path: str, revision: str):
    ...
    if model_class == "CLIPTextModel":
        from transformers import CLIPTextModel; return CLIPTextModel
    elif model_class == "RobertaSeriesModelWithTransformation":
        from diffusers.pipelines.alt_diffusion.modeling_roberta_series import RobertaSeriesModelWithTransformation; return RobertaSeriesModelWithTransformation
    elif model_class == "T5EncoderModel":
        from transformers import T5EncoderModel; return T5EncoderModel
    else:
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--out_dir", type=str, default="text-inversion-model",
    ...
    parser.add_argument("--ckpting_steps", type=int, default=500,
        help="Save a checkpoint of the training state every X updates. Checkpoints can be used for resuming training via `--resume_from_ckpt`. In the case that the checkpoint is better than the final trained model, the checkpoint can also be used for inference. Using a checkpoint for inference requires separate loading of the original pipeline and the individual checkpointed model components. See https://huggingface.co/docs/diffusers/main/en/training/dreambooth#performing-inference-using-a-saved-checkpoint for step by step instructions.")
    parser.add_argument("--ckpts_total_limit", type=int, default=None,
        help="Max number of checkpoints to store. Passed as `total_limit` to the `Accelerator` `ProjectConfiguration`. See Accelerator::save_state https://huggingface.co/docs/accelerate/package_reference/accelerator#accelerate.Accelerator.save_state for more details")
    ...
    parser.add_argument("--adam_eps", type=float, default=1e-08, help="Epsilon value for the Adam optimizer")
    ...
    parser.add_argument("--val_steps", type=int, default=100,
        help="Run validation every X steps. Validation consists of running the prompt `args.val_prompt` multiple times: `args.num_val_imgs` and logging the images.")
    ...
    parser.add_argument("--set_grads_to_none", action="store_true",
        help="Save more memory by using setting grads to None instead of zero. Be aware, that this changes certain behaviors, so disable this argument if it causes any problems. More info: https://pytorch.org/docs/stable/generated/torch.optim.Optimizer.zero_grad.html")
    ...
    parser.add_argument("--save_model_weights", default=False, action="store_true",
        help="Whether to save weights/checkpoint after training is done")
...
class PDMBoothDataset(Dataset):
    ...
    def __init__(
        ...
        tokenizer_max_length=None, inst_features_root: str = None, save_pdm_fs: bool = False, do_extract_pdm_fs: bool = False, sam_processor: SamProcessor = None, sam_model: SamModel = None,
...
def main(args):
    ...
    if args.seed is not None: # If passed along, set the training seed now.
        set_seed(args.seed)
    ...
    if args.use_prior_loss: # Generate class images if prior preservation is enabled.
        ...
        if cur_cls_imgs < args.num_cls_imgs:
            ...
            pipe = DiffusionPipeline.from_pretrained(args.pretrained_model_name_or_path,
                torch_dtype=torch_dtype, safety_checker=None, revision=args.revision)
            pipe.set_progress_bar_config(disable=True)
    ...
    text_encoder_cls = import_model_class_from_model_name_or_path( # import correct text encoder class
    ...
    def save_model_hook(models, weights, out_dir):
        for model in models:
            sub_dir = "unet" if isinstance(model, type(acc.unwrap_model(unet))) else "text_encoder"
            model.save_pretrained(os.path.join(out_dir, sub_dir))
    ...
    def load_model_hook(models, input_dir):
        while len(models) > 0:
            ...
            if isinstance(model, type(acc.unwrap_model(text_encoder))): # load transformers style into model
                load_model = text_encoder_cls.from_pretrained(input_dir, subfolder="text_encoder")
                model.config = load_model.config
            else: # load diffusers style into model
    ...
    if acc.unwrap_model(unet).dtype != torch.float32:
        raise ValueError(
            f"Unet loaded as datatype {acc.unwrap_model(unet).dtype}. {low_precision_error_string}")
    if args.train_text_encoder and acc.unwrap_model(text_encoder).dtype != torch.float32:
        raise ValueError(f"Text encoder loaded as datatype {acc.unwrap_model(text_encoder).dtype}."
    ...
    if args.use_8bit_adam: # Use 8-bit Adam for lower memory usage or to fine-tune the model in 16GB GPUs
        try:
    ...
    if args.precompute_text_embeds:
        ...
        precomputed_encoder_hidden_states = compute_text_embeddings(args.inst_prompt)
        val_prompt_negative_prompt_embeds = compute_text_embeddings("")

        if args.val_prompt is not None:
            val_prompt_encoder_hidden_states = compute_text_embeddings(args.val_prompt)
        else:
            val_prompt_encoder_hidden_states = None
        if args.inst_prompt is not None:
            precomputed_inst_prompt_encoder_hidden_states = compute_text_embeddings(args.inst_prompt)
        else:
    ...
    ds = PDMBoothDataset( # Setup Dataset and DataLoaders
        ...
        encoder_hidden_states=precomputed_encoder_hidden_states,
    ...
    lr_scheduler = get_scheduler(args.lr_scheduler, optimizer=opt, num_cycles=args.lr_num_cycles,
        num_warmup_steps=args.lr_warmup_steps * args.gradient_accumulation_steps, power=args.lr_power,
        num_training_steps=args.max_train_steps * args.gradient_accumulation_steps)

    if args.train_text_encoder:
        unet, text_encoder, opt, dl, lr_scheduler = acc.prepare(unet, text_encoder, opt, dl, lr_scheduler)
    else:
        unet, opt, dl, lr_scheduler = acc.prepare(unet, opt, dl, lr_scheduler)

    # For mixed-precision training we cast all non-trainable weigths (vae, non-lora text_encoder and non-lora unet) to half-precision as these weights are only used for inference (keeping weights in full precision is not required).
    ...
    # need to recalc total training steps as the size of the training dataloader may have changed.
    num_update_steps_per_epoch = math.ceil(len(dl) / args.gradient_accumulation_steps)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(dl):
            # Skip steps until reaches the resumed step
            ...
            with acc.accumulate(unet):
                ...
                if args.offset_noise: # Sample noise that we'll add to the model input
                ...
                opt.step(); lr_scheduler.step(); opt.zero_grad(set_to_none=args.set_grads_to_none)

            if acc.sync_gradients: # Checks if accelerator performed an optimization step behind the scenes
                prog_bar.update(1)
                global_step += 1
                if acc.is_main_process:
                    if args.save_model_weights and global_step % args.ckpting_steps == 0:
                    ...
                    imgs = []
                    if args.val_prompt is not None and global_step % args.val_steps == 0:
                        imgs = log_validation(
                            text_encoder, tokenizer, unet, vae, args, acc, weight_dtype,
                            epoch, val_prompt_encoder_hidden_states,val_prompt_negative_prompt_embeds)
